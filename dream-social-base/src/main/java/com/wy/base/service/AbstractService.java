...
public abstract class AbstractService<M extends BaseMapper<T>, T, ID extends Serializable>
	...
	/** 存储反射过程中使用的类和字段 */
	private static final Map<Class<?>, List<Field>> CACHE_CLASS_FIELDS = new ConcurrentHashMap<>(128);
	...
	protected void checkUniqueAndSort(T t, boolean saveOrUpdate, boolean checkSort) {
		...
		while (Object.class != searchType && searchType != null) {
			List<Field> fields = CACHE_CLASS_FIELDS.get(searchType);
			if (ListTool.isEmpty(fields)) {
				fields = Arrays.asList(searchType.getDeclaredFields());
				CACHE_CLASS_FIELDS.put(searchType, (fields.size() == 0 ? Collections.emptyList() : fields));
			}
	...
	protected void checkUnique(T t, Field field) {
		...
		if (hasValue(JSON.parseObject(JSON.toJSONString(temp), entityClass))) {
	 ...
	 */
	@Override
	public Object creates(List<T> ts) {
	...
	public Result<List<T>> getEntitys(T t) {
		...
		if (StrTool.isNotBlank(pager.getPageOrder())) {
			if (StrTool.equalsIgnoreCase(Direction.ASC.name(), pager.getPageDirection())) {
		...
		return null == page || ListTool.isEmpty(page.getRecords()) ? Result.ok()
	...
	public Result<List<Map<String, Object>>> getLists(Map<String, Object> params) {
		if (Objects.isNull(params)) {
			return Result.ok(super.listMaps());
		}
		if (Objects.nonNull(params.get("pageIndex"))) {
			int pageIndex = NumberTool.toInt(params.get("pageIndex"));
			int pageSize = Constant.PAGE_SIZE;
			if (pageIndex <= 0) {
				return Result.ok(super.listMaps());
			} else {
				if (Objects.nonNull(params.get("pageSize"))) {
					int tempPageSize = NumberTool.toInt(params.get("pageSize"));
					if (tempPageSize > 0) {
						pageSize = tempPageSize;
					}
				}
			}
			Page<Map<String, Object>> pageMaps = super.pageMaps(new Page<Map<String, Object>>(pageIndex, pageSize),
					new QueryWrapper<T>(JSON.parseObject(JSON.toJSONString(params), entityClass)));
			return Objects.isNull(pageMaps) || ListTool.isEmpty(pageMaps.getRecords()) ? Result.ok()
					: Result.page(pageMaps.getRecords(), pageIndex, pageSize, pageMaps.getTotal());

		}
		return Result.ok(super.listMaps(new QueryWrapper<T>(JSON.parseObject(JSON.toJSONString(params), entityClass))));
	}

	/**
	 * 获得指定字段的最大值,只能是数字或时间类型
	 * 
	...
	/**
	 * FIXME 获得指定字段的最大值,只能是数字或时间类型 未测试
	 * 
	 * @param column Java属性字段
	 * @param hump2Snake 是否驼峰转下划线:true->转,false->不转
	 * @return 最大值
	 */
	public Long getMax(String column, boolean hump2Snake) {
		Field field = checkFieldType(column);
		String snakeColumn = hump2Snake ? StrTool.hump2Snake(column) : column;
		Page<Map<String, Object>> pageParam = new Page<>(1, 1);
		pageParam.addOrder(OrderItem.desc(snakeColumn));
		Page<Map<String, Object>> pageMaps = pageMaps(pageParam);
		if (Objects.isNull(pageMaps) || ListTool.isEmpty(pageMaps.getRecords()) || pageMaps.getTotal() == 0) {
			return 0l;
		}
		if (ClassTool.isAssignable(field.getType(), Number.class)) {
			return Long.parseLong(pageMaps.getRecords().get(0).get(snakeColumn) + "");
		} else {
			Date date = (Date) (pageMaps.getRecords().get(0).get(snakeColumn));
			return date.getTime();
		}
	}

	protected Field checkFieldType(String column) {
		try {
			Field field = entityClass.getDeclaredField(column);
			ReflectTool.fixAccessible(field);
			if (ClassTool.isIntegral(field.getType()) || field.getType() == Date.class
					|| ClassTool.isAssignable(field.getType(), TemporalAdjuster.class)) {
				return field;
			} else {
				throw new ResultException("该字段不是一个数字类型或时间类型");
			}
	...
	@Override
	public List<Tree> getTree(ID id, Boolean self, Map<String, Object> params) {
		// TODO Auto-generated method stub
		return null;
	}
	 ...
	 */
	@Override
	public Object infos(List<ID> ids) {
